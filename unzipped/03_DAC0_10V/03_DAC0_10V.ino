/*Hankerila  ADI16 uses the MCP4725 to output a voltage ranging from 0 to 10 volts.*/
...
void setVoltage2(int voltage_mV2) {
    int val2 = map(voltage_mV2, 0, 5000, 0, 4095);
    Wire.beginTransmission(MCP4725_ADDR2);
    Wire.write((val2 >> 8) & 0xFF);
...
void loop() {
    ...
    for (int voltage_mV = 0; voltage_mV <= 5000; voltage_mV += 100) {   
        ...
        Serial.print("Set voltage1 to: ");
        Serial.print(voltage_mV*2);
        Serial.println(" mV");
        delay(500); 
    }
    for (int voltage_mV2 = 0; voltage_mV2 <= 5000; voltage_mV2 += 100) {
        setVoltage2(voltage_mV2);
        Serial.print("Set voltage2 to: ");
        Serial.print(voltage_mV2*2);
        Serial.println(" mV");
        delay(500); 
    }
}
